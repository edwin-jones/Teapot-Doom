...
{ 
	...
	//always update gamepad rumble per game tic.
	UpdateRumble();
    ...
    // on the keyboard and joystick AND gamepad
    if (joyxmove < 0
	|| joyxmove > 0  
	|| gamekeydown[key_right]
	|| gamekeydown[key_left]
	|| IsAnalogStickPressedInDirection(RIGHT, LEFT)
	|| IsAnalogStickPressedInDirection(RIGHT, RIGHT))
	turnheld += ticdup; 
    else 
	turnheld = 0; 
    ...
    { 
		if (gamekeydown[key_right] || IsAnalogStickPressedInDirection(RIGHT, RIGHT))
	    cmd->angleturn -= angleturn[tspeed]; 
		if (gamekeydown[key_left] || IsAnalogStickPressedInDirection(RIGHT, LEFT))
	    cmd->angleturn += angleturn[tspeed]; 
	if (joyxmove > 0) 
	    cmd->angleturn -= angleturn[tspeed]; 
	if (joyxmove < 0) 
	...
	if (gamekeydown[key_up] || IsAnalogStickPressedInDirection(LEFT, UP))
    {
	// fprintf(stderr, "up\n");
	forward += forwardmove[speed]; 
    }
	if (gamekeydown[key_down] || IsAnalogStickPressedInDirection(LEFT, DOWN))
    {
	// fprintf(stderr, "down\n");
	forward -= forwardmove[speed]; 
    }
    ...
    if (gamekeydown[key_strafeleft]
     ...
     || joystrafemove < 0
	 || IsAnalogStickPressedInDirection(LEFT, LEFT))
    {
        side -= sidemove[speed];
    }
    ...
    if (gamekeydown[key_straferight]
     ...
     || joystrafemove > 0
	 || IsAnalogStickPressedInDirection(LEFT, RIGHT))
    {
        side += sidemove[speed]; 
    }
	...
	if (gamekeydown[key_fire] || mousebuttons[mousebfire]
		|| joybuttons[joybfire] || (IsTriggerDown(RIGHT)))
	{
    ...
    if (gamekeydown[key_use]
	 ...
	 || IsButtonPressed(XINPUT_GAMEPAD_A))
    { 
	cmd->buttons |= BT_USE;
	...
	if (IsButtonPressed(XINPUT_GAMEPAD_LEFT_SHOULDER))
	{
		next_weapon = 1;
	}

	if (IsButtonPressed(XINPUT_GAMEPAD_RIGHT_SHOULDER))
	{
		next_weapon = -1;
	}
